use crate::{
    mir::{MIRBlock, MIRInstruction, MIRType, MIRBranch},
    parser::parse_nodes::{declarations::{DeclarationSpecifiers, Declarator, Initializer}, expressions::CExpression}, environment_builder::ext_type::ExtType,
};
...
impl EnvironmentController {
    pub(crate) fn handle_if_statement(
        ...
        // controlling value
        let cond_value = self.walk_expression(ctx, controlling_expr.clone(), &ExtType::Int { is_const: false, is_volatile: false, signed: true, size: 4 }.into_pretty());
        ...
        current_block.borrow_mut().branches = Some((cond_value,vec![
            MIRBranch{ is_default: false, value_needed: 1, to_block: if_true_block_id }
        ]));
        ...
        if else_block_id_link != -1{
            if let Some(current_block_branches) = &mut current_block.borrow_mut().branches{
                current_block_branches.1.push(MIRBranch{ is_default: true, value_needed: 0, to_block: else_block_id_link as usize });
            }else{
                panic!()
            }
        }else if let Some(current_block_branches) = &mut current_block.borrow_mut().branches{
            current_block_branches.1.push(MIRBranch{ is_default: true, value_needed: 0, to_block: merge_block_id as usize });
        }else{
            panic!()
        }
