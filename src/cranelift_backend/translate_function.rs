...
use crate::mir::{MIRBlock, MIRFunction, MIRInstruction, MIRType, MIRValue};
...
impl CraneliftBackend {
    pub(crate) fn translate_function(&mut self, input: MIRFunction) {
            ...
            .enumerate()
        {
            ...
            let data =
                StackSlotData::new(StackSlotKind::ExplicitSlot, param_type.get_size_in_bytes());
            let var_slot = builder.create_sized_stack_slot(data);
            builder.ins().stack_store(val, var_slot, 0);
            if let std::collections::hash_map::Entry::Vacant(e) =
                var_stack_slot_map.entry(param.to_string())
            {
                e.insert(var_slot);
                var_idx_counter += 1;
            }
        ...
        for var_local_ref_pair in &input.var_name_id_map {
            ...
            let data =
                StackSlotData::new(StackSlotKind::ExplicitSlot, var_type.get_size_in_bytes());
            let var_slot = builder.create_sized_stack_slot(data);
            if let std::collections::hash_map::Entry::Vacant(e) =
...
impl CraneliftFunctionTranslator<'_> {
    pub(crate) fn translate_block(
        &mut self,
        current_block: Rc<RefCell<MIRBlock>>,
        block_idx: usize,
    ) {
        ...
        if block_idx != 0 {
            ...
            debug!(
                "block: {:?},func.is_filled: {}",
                block_idx,
                self.func_builder.is_filled()
            );
        }
        ...
        if let Some(branches) = &current_block.branches {
            ...
            for branch in &branches.1 {
                ...
                if branch.is_default {
                    self.func_builder
                        .ins()
                        .jump(*self.block_map.get(&branch.to_block).unwrap(), &[]);
                } else {
                    self.func_builder.ins().brnz(
                        cond_value,
                        *self.block_map.get(&branch.to_block).unwrap(),
                        &[],
                    );
                }
            }
    ...
    pub(crate) fn translate_instruction(&mut self, instr: MIRInstruction) {
        match instr {
            ...
            MIRInstruction::ReadLocal(mir_output_value, mir_local_ref) => {
                ...
                // use var
                let cranelift_output_value = self.func_builder.ins().stack_load(
                    variable_type.into_cranelift_type(),
                    *variable_slot,
                    0,
                );
            ...
            MIRInstruction::AssignLocal(mir_local_ref, mir_value_to_assign_to) => {
                ...
                let cranelift_assign_value =
                    self.mir_value_to_cranelift_value(mir_value_to_assign_to);

                self.func_builder
                    .ins()
                    .stack_store(cranelift_assign_value, *variable_slot, 0);
            }
            MIRInstruction::IntMath(result_mir_value, left_mir_value, right_mir_value, op_kind) => {
                ...
                let math_res_value = match op_kind {
                    crate::mir::IntMathKind::Add => {
                        self.func_builder.ins().iadd(left_value, right_value)
                    }
                    crate::mir::IntMathKind::Sub => {
                        self.func_builder.ins().isub(left_value, right_value)
                    }
                    crate::mir::IntMathKind::Mul => {
                        self.func_builder.ins().imul(left_value, right_value)
                    }
                    crate::mir::IntMathKind::Div => {
                        self.func_builder.ins().sdiv(left_value, right_value)
                    } // TODO: this stuff needs to be signed dependent
            ...
            MIRInstruction::Compare(compare_result, left_mir_value, right_mir_value, cmp_kind) => {
                let left_value = self.mir_value_to_cranelift_value(left_mir_value);
                let right_value = self.mir_value_to_cranelift_value(right_mir_value);
                let cmp_value = self.func_builder.ins().icmp(
                    match cmp_kind {
                        crate::mir::IntCmpKind::Eq => IntCC::Equal,
                        crate::mir::IntCmpKind::UnEq => IntCC::NotEqual,
                        crate::mir::IntCmpKind::GT => IntCC::SignedGreaterThan,
                        crate::mir::IntCmpKind::LT => IntCC::SignedLessThan,
                        crate::mir::IntCmpKind::GET => IntCC::SignedGreaterThanOrEqual,
                        crate::mir::IntCmpKind::LET => IntCC::SignedLessThanOrEqual,
                    },
                    left_value,
                    right_value,
                );
                let cmp_value = self.func_builder.ins().bint(
                    self.mir_function
                        .value_type_map
                        .get(&compare_result)
                        .unwrap()
            ...
            MIRInstruction::GetAddrOfLocal(ouput_result, local_ref) => {
                let var_name = self
                    .mir_function
                    .var_name_id_map
                    .get_by_left(&local_ref)
                    .unwrap();
                let var_stack_slot = self.var_stack_slot_map.get(var_name).unwrap();
                let ref_value = self.func_builder.ins().stack_addr(
            ...
            MIRInstruction::Deref(output_res, value_to_deref, wanted_deref_type) => {
                ...
                let value = self.func_builder.ins().load(
                    wanted_deref_type.into_cranelift_type(),
                    MemFlags::new(),
                    cranelift_value_to_deref,
                    0,
                );
